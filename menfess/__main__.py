...
from menfess.dk import Dk, data
from menfess import dbclone, bots, set_main_bot
from menfess.helpers import Database
from menfess.database.referral import reset_referrals
import pytz
...
async def main():
    ...
    try:
        ...
        scheduler.start()
        _, jieh = await Dk(0, memory=False).start()
        ...
        for clone in all_clone:
            try:
                if not clone.status:
                    continue
                sts, jiehbot = await Dk(owner_id=clone.user_id, token=clone.token, memory=False, owner=clone).start()
                if not sts:
                    continue
                bots.append(jiehbot)
