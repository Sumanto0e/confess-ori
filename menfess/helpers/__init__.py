from menfess.database import Database
...
class Helper:
    ...
    async def cek_langganan_channel(self, user_id: int):
        if user_id == config.id_admin:
            return True
        try:
            member = await self.bot.get_chat_member(self.bot.channel_1, user_id)
        except UserNotParticipant:
            return False
        try:
            member = await self.bot.get_chat_member(self.bot.channel_2, user_id)
        except UserNotParticipant:
            return False
    ...
    async def cek_join_channel(self, user_id: int):
        if user_id == config.id_admin:
            return True
        try:
            member = await self.bot.get_chat_member(self.bot.channel_1, user_id)
    ...
    async def pesan_langganan(self):
        link_1 = await self.bot.export_chat_invite_link(self.bot.channel_1)
        link_2 = await self.bot.export_chat_invite_link(self.bot.channel_2)
    ...
    async def daftar_pelanggan(self):
        database = Database(self.user_id, self.bot.me.id)
    ...
    async def send_to_channel_log(self, type: str = None, link: str = None):
        if type == 'log_daftar':
            ...
            pesan = "Jangan Lupa Main Bot @ONSBASEBOT"
        await self.bot.send_message(self.bot.channel_log, pesan, enums.ParseMode.HTML, disable_web_page_preview=True)
...
async def isUser(userid: int):
    database = Database(userid)
    if not await database.cek_user():
        return "Kamu tidak bisa membuka link ini, silahkan daftar dulu ke bot @CONFESONSBOT"
    return True



async def isAuth(filter, client, update: Message):
    if update.from_user.id == config.id_admin:
        return True
    if client.owner_id == update.from_user.id:
        return True
    return False

isAdmin = filters.create(isAuth)