...
@Bot("daddysugar")
async def on_daddy_sugar_handler(client: Client, msg: types.Message, db: Database = None):
    daddy_sugar = db.get_data_bot(client.me.id).daddy_sugar
        ...
        for i in top_rate:
            ...
            for j in top_id:
                if daddy_sugar[j]['rate'] == i:
                    ...
                    top_id.remove(j)
                    index += 1
        pesan += f"\nmenampilkan Real daddy sugar yang trusted membiayai wanita wanita di ons"
        await msg.reply(pesan, True, enums.ParseMode.HTML)


@Bot(r"^[\/]addsugar(\s|\n)*(\d+)$", regex=True)
async def tambah_sugar_daddy_handler(client: Client, msg: types.Message, db: Database = None):
    ...
    if await db.cek_user_didatabase():
        status = [
            ...
            'proplayer', 'girlfriend rent', 'boyfriend rent'
        ]
        ...
        try:
            ...
            await db.tambah_sugar_daddy(int(target), client.me.id, nama)
            return await msg.reply_text(
