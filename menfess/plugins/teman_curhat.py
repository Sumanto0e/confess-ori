...
@Bot("temancurhat", is_admin=True)
async def teman_curhat_handler(client: Client, msg: Message, db: Database = None):
    db = Database(msg.from_user.id, client.me.id)
    teman_curhat = db.get_data_bot(client.me.id).temancurhat
...
@Bot(r"^[\/]addtemancurhat", regex=True, is_admin=True)
async def tambah_teman_curhat_handler(client: Client, msg: Message, db: Database = None):
    ...
    if y:
        ...
        if await db.cek_user_didatabase():
            ...
            try:
                ...
                await db.tambah_teman_curhat(int(target), client.me.id, nama)
                return await msg.reply_text(
