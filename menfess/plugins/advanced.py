...
@Bot("setchannelpost")
async def on_setchannelpost_handler(client: Dk, message: Message, db: Database = None):
...
async def on_setchannelorgrub_handler(client: Dk, message: Message, db: Database = None):
    if client.owner_id != 0:
        owner = Clone(client.owner_id)
        clone = await owner.get_clone()
        if clone is None:
...
@Bot("setchannellog")
async def on_setchannellog_handler(client: Dk, message: Message, db: Database = None):
    if client.owner_id != 0:
        owner = Clone(client.owner_id)
        clone = await owner.get_clone()
        if clone is None:
...
@Bot("setdailysend")
async def on_setdailysend_handler(client: Dk, message: Message, db: Database = None):
    if client.owner_id != 0:
        owner = Clone(client.owner_id)
        clone = await owner.get_clone()
        if clone is None:
...
async def on_setbiayasendmenfes_handler(client: Dk, message: Message, db: Database = None):
    if client.owner_id != 0:
        owner = Clone(client.owner_id)
        clone = await owner.get_clone()
        if clone is None:
...
async def on_setbiayahapusmenfes_handler(client: Dk, message: Message, db: Database = None):
    if client.owner_id != 0:
        owner = Clone(client.owner_id)
        clone = await owner.get_clone()
        if clone is None:
...
async def on_setbiayapinnedmenfes_handler(client: Dk, message: Message, db: Database = None):
    if client.owner_id != 0:
        owner = Clone(client.owner_id)
        clone = await owner.get_clone()
        if clone is None:
