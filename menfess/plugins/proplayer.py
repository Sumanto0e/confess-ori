...
@Bot("proplayer")
async def on_pro_player_handler(client: Client, msg: Message, db: Database = None):
    pro_player = db.get_data_bot(client.me.id).proplayer
...
@Bot(r"^[\/]addproplayer", regex=True, is_admin=True)
async def tambah_pro_player_handler(client: Client, msg: Message, db: Database = None):
    ...
    if y:
        ...
        if await db.cek_user_didatabase():
            ...
            try:
                ...
                await db.tambah_pro_player(int(target), client.me.id, nama)
                return await msg.reply_text(
